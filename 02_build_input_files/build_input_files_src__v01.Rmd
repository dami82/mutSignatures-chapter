...
de_novo_fld <- '../03_method/VCF_de_novo/'
deconv_fld <- '../03_method/VCF_deconv/'
extra_fld <- '../03_method/VCF_extra/'
...
for (fi in btch_01) {
  ...
  file.copy(from = paste0(out_fld, fii), 
            to = paste0(de_novo_fld, fii), 
            overwrite = TRUE)  
}

# Move some VCF files to folder for de-convolution
#
btch_02 <- c('A2-A0SW', 'A2-A0T5', 'A8-A084', '', 'A8-A092', 'A8-A093', 
             'A8-A093', 'A8-A09Q', 'A8-A09Z', '', 'AC-A2QI', 'AC-A62X', 
             'AN-A0AK', 'BH-A0W7', 'BH-A18G', 'BH-A1F6', 'BH-A209', 'C8-A26V', 
             'C8-A26Y', 'C8-A3M7', 'D8-A1XJ', 'D8-A1XJ', 'D8-A1XZ', 'E2-A109', 
             'E2-A1IN', 'E2-A1LG', 'EW-A1J5', 'GM-A2D9', 'GM-A2DO', 'LL-A5YL')
...
#, 'D8-A27G', 'D8-A27M', 'E9-A1RF', 'EW-A1IZ', 'EW-A1J5', 'EW-A1P4', 'EW-A1PC', 'GI-A2C8')

for (fi in btch_02) {
  fii <- grep(gsub('[[:punct:]]', '_', fi), dir(out_fld), value = TRUE)
  file.copy(from = paste0(out_fld, fii), 
            to = paste0(deconv_fld, fii), 
            overwrite = TRUE)  
}

# Extra
fii <- grep('LL_A5YL', dir(out_fld), value = TRUE)
fic <- file(paste0(out_fld, fii), open = 'r')
y <- list()
zz <- tryCatch({
  while(1){
    tli <- readLines(con = fic, n = 1)  
    if(grepl('^chr', tli)) 
      tli <- sub('^chr', '', tli)
    y[[length(y) + 1]] <- tli
  }
}, error = function(e) { NULL }, finally = {close(fic)})
writeLines(text = do.call(c, y), con = paste0(extra_fld, 'example_01.vcf'))
