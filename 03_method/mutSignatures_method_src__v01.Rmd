...
- Data import from bladder cancer VCF files and data pre-processing
...
### Data import from VCF files and data pre-processing
...
x %>% 
  head() %>% kable() %>% 
  kable_styling(bootstrap_options = "striped", font_size = 7, full_width = FALSE)
```
...
blca_params <- setMutClusterParams( 
    ...
    num_totIterations = 30,          # bootstrapping: usually 500-1000
...
de_novo_sigs_df %>% 
  head(12) %>% kable() %>% 
  kable_styling(bootstrap_options = "striped", font_size = 8, full_width = FALSE)
```
