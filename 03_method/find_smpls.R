...
#
vcf_ext <- 'vcf.gz'
de_novo_fld <- '../02_build_input_files/VCF/'
...
#
blca_analysis <- 
  decipherMutationalProcesses(input = blca_counts,
                              params = blca_params)
...
matchSignatures(blca_analysis$Results$signatures, blca_cosmix)


zz <- mutSignatures::resolveMutSignatures(mutCountData = blca_counts, 
                                          signFreqData = blca_cosmix, 
                                          byFreq = TRUE)


zx <- mutSignatures::as.data.frame(zz$results$freq.result, transpose = TRUE)

unique(c(rownames(head(zx[order(zx$COSMIC.2, decreasing = T), ], 18)), 
         rownames(head(zx[order(zx$COSMIC.13, decreasing = T), ], 18)), 
         rownames(head(zx[order(zx$COSMIC.5, decreasing = T), ], 18)), 
         rownames(head(zx[order(zx$COSMIC.1, decreasing = T), ], 18))))


zz2 <- mutSignatures::as.data.frame(zz$results$freq.result, transpose = TRUE)
zz2 %>% arrange(desc(.$Sign.01)) %>% head()
